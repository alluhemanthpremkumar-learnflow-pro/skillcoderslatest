...
const restrictedPatterns = [
  ...
  // More aggressive alphabetic numbers
  /\b(one|two|three|four|five|six|seven|eight|nine|zero)\b/gi,
  /([a-zA-Z]*\d[a-zA-Z]*){5,}/g, // 5 or more digits mixed with letters
];
...
const ChatRoom = ({ isOpen, onClose, userRole = 'student', currentUsername = 'CurrentUser', meetingId }: ChatRoomProps) => {
  ...
  const handleRemoveMember = (memberId: string) => {
    ...
    // In a real app we'd setGroups(updatedGroups), but here we simulate by updating selected
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1 mt-2">
                      {group.members.map(member => (
                        <div key={member} className="flex justify-between items-center text-xs text-muted-foreground pl-2 bg-background/30 rounded py-1 pr-1">
                          <span className="truncate">{member}</span>
                          {(userRole === 'teacher' || userRole === 'admin') && member.startsWith('student') && (
                            <Button 
                              size="icon" 
                              variant="ghost" 
                  ...
                  >
                    <div className={`flex-1 max-w-[70%] ${
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {/* Delete Button for Admins/Teachers on Student messages */}
                        {(userRole === 'admin' || userRole === 'teacher') && message.role === 'student' && (
                          <Button
                            size="icon"
                            variant="ghost"
                            className="h-5 w-5 ml-auto text-destructive hover:bg-destructive/10"

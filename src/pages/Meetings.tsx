...
const Meetings = () => {
  ...
  const handleOpenChat = (meetingId: string, isEnrolled: boolean) => {
    if (!isEnrolled) {
      alert('You must be enrolled in this course to chat with instructors.');
      return;
    }
      ...
      <GlowCard glowColor={meeting.type === 'class' ? 'cyan' : 'purple'}>
        ...
        <div className="flex gap-3">
          <Button
            variant="outline"
            className="flex-1"
            onClick={() => handleOpenChat(meeting.id, meeting.isEnrolled)}
            disabled={!meeting.isEnrolled && userRole === 'student'}
            title={!meeting.isEnrolled && userRole === 'student' ? 'Enroll in this course to chat' : 'Ask doubts from instructor'}
          >
